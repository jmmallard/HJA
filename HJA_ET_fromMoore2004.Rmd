...
transEx <- left_join(gfPRI, trans2000, by = "DATE")

#SPREAD BY TREE SPECIES

#Plot bivariates of transpiration against radiation and against vpd
transRadPlot <- transEx %>% 
  select(DATE, trans, Rad) %>% 
  drop_na() %>% 
  ggplot(aes(x = Rad, y = trans)) + geom_point()
print(transRadPlot)

transVpdPlot <- transEx %>% 
  select(DATE, trans, Vpd) %>% 
  drop_na() %>% 
  ggplot(aes(x = Vpd, y = trans)) + geom_point()
print(transVpdPlot)
```
