...
TransDef <- sapFluxAllPred %>% 
  ...
  mutate(transDefHill = transHillYoung - transHillOld,
         transDefRip = transRipYoung - transRipOld) %>% 
  mutate(transOld = transHillOld + transRipOld,
         transYoung = transHillYoung + transRipYoung) %>% 
  mutate(transDef = transYoung - transOld) %>% 
...
sum(TransDef$transDefHill[between(TransDef$DATE, as.Date("2000-05-09"), as.Date("2000-06-10"))])
