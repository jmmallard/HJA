...
sapFlux1999 <- Figure3bData %>% 
  ...
  filter(!between(DATE, as.Date("1999-07-29"), as.Date("1999-08-04"))) #Data gap in figure 3b. Interpolated later
...
#   select(DATE, PSMEo, PSMEy, Rad) %>%
#   drop_na() %>%
#   ggplot() +
#   geom_point(aes(x = Rad, y = PSMEo, color = "PSMEo")) +
#   geom_point(aes(x = Rad, y = PSMEy, color = "PSMEy")) +
#   labs(y = "Trans")
# print(transRadPlot)
# 
# transVpdPlot <- transEx %>%
#   select(DATE, PSMEo, PSMEy, Vpd) %>%
#   drop_na() %>%
#   ggplot() +
#   geom_point(aes(x = Vpd, y = PSMEo, color = "PSMEo")) +
#   geom_point(aes(x = Vpd, y = PSMEy, color = "PSMEy")) +
#   trans(y = "Trans")
# print(transVpdPlot)

# Generate a bunch of regression models. 
# RegTransPSMEoVpd <- lm(PSMEo ~ Vpd, data = transEx)
# RegTransPSMEoRad <- lm(PSMEo ~ Rad, data = transEx)
# RegTransPSMEoVpdRad <- lm(PSMEo ~ Rad + Vpd, data = transEx)
# RegTransPSMEyVpd <- lm(PSMEy ~ Vpd, data = transEx)
# RegTransPSMEyRad <- lm(PSMEy ~ Rad, data = transEx)
# RegTransPSMEyVpdRad <- lm(PSMEy ~ Rad + Vpd, data = transEx)
# 
# summary(RegTransPSMEoVpd)
# summary(RegTransPSMEoRad)
# summary(RegTransPSMEoVpdRad)
# summary(RegTransPSMEyVpd)
# summary(RegTransPSMEyRad)
# summary(RegTransPSMEyVpdRad)
...
transExPred <- transEx %>% 
  add_predictions(model = RegTransPSMEoRad, var = "PSMEoPred") %>% 
  add_predictions(model = RegTransPSMEyRad, var = "PSMEyPred") %>% 
  mutate(Extrap = ifelse(is.na(PSMEo),1,0)) %>% 
  mutate(PSMEo = ifelse(is.na(PSMEo), PSMEoPred, PSMEo)) %>% 
  mutate(PSMEy = ifelse(is.na(PSMEy), PSMEyPred, PSMEy)) 

transPredPSMEyPlot <- transExPred %>%  
  filter(between(DATE, as.Date("2000-04-01"), as.Date("2000-10-31"))) %>% 
  ggplot() +
  geom_line(aes(x = DATE, y = PSMEy, color = Extrap))
print(transPredPSMEyPlot)

transPredPSMEoPlot <- transExPred %>%  
  filter(between(DATE, as.Date("2000-04-01"), as.Date("2000-10-31"))) %>% 
  ggplot() +
  geom_line(aes(x = DATE, y = PSMEo, color = Extrap))
print(transPredPSMEoPlot)
