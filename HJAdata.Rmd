...
- Calculate estimated monthly PET from Thornwaite method: https://en.wikipedia.org/wiki/Potential_evaporation for example
  ...
  - Td: Average daily temperature in the month
...
monthData <- allData %>% 
  ...
  mutate(dayLength = chillR::daylength(hjaLat,doy)[[3]]) %>% 
  group_by(month = floor_date(DATE, "month")) %>% 
  summarise(meanMonTemp = mean(meanTemp, na.rm = "TRUE"),
            meanDaylength = mean(dayLength, na.rm = "TRUE")) %>% 
  na.omit %>% 
  mutate(monthDays = as.numeric(days_in_month(month)))
...
- Input data is daily PET, runoff, and precip
